"""
FastAPI route handlers for /targets.

Owns CRUD for EndpointTarget (create, list, get by id, partial update)
plus a read-only nested route for a target's recent CheckResult history.
CheckResult rows themselves are written elsewhere (the scheduler/checker
flow), not through this router; this file only reads them back.
"""
...
def target_post(
    ...
    # By Rest Convention returns 201 on creation
...
def targets_all_get(db: Session = Depends(get_db)):
    statement = select(
        EndpointTarget
    )   # We create the query on our SQLAlchemy model
...
def target_id_patch(
    target_update: TargetUpdate, target_id: int, db: Session = Depends(get_db)
):
...
def result_id_get(target_id: int, db: Session = Depends(get_db)):
    # Capped at 100 to avoid unbounded response size on high-frequency targets.
    # No pagination yet
    statement = (
        select(CheckResult)
